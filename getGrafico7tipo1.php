<?php
    ...
    { 
        ...
        $dataSeries["toolTipContent"]="Ditta: {name}<br>Mese: {x}<br>Media operatori: {y}";
        $dataSeries["showInLegend"]=true;
        $dataSeries["markerSize"]=0;
        $dataSeries["dataPoints"]=[];

        $query2="SELECT TOP (100) PERCENT t3.anno, t3.mese, t3.nome AS ditta, CONVERT(FLOAT, t3.nOperatori) / CONVERT(FLOAT, t4.nGiorni) AS mediaOperatori
                FROM (SELECT nome, anno, mese, COUNT(operatore) AS nOperatori
                ...
                AS mese, dbo.cantiere_ponti_ditte_registrazioni.ponte, dbo.cantiere_ponti_ditte_registrazioni.operatore
                ...
                WHERE (dbo.cantiere_registrazioni.commessa = ".$_SESSION['id_commessa'].") AND (dbo.cantiere_ponti_ditte_registrazioni.ponte IN ('".$inPonti."')) AND (DATEPART(yy, 
    ...
    $query3="SELECT mese, AVG (mediaOperatori) AS mediaOperatori FROM ( SELECT TOP (100) PERCENT t3.anno, t3.mese, t3.nome AS ditta, CONVERT(FLOAT, t3.nOperatori) / CONVERT(FLOAT, t4.nGiorni) AS mediaOperatori
            FROM (SELECT nome, anno, mese, COUNT(operatore) AS nOperatori
            FROM (SELECT DISTINCT 
            dbo.cantiere_registrazioni.data, dbo.cantiere_ditte.nome, dbo.cantiere_registrazioni.commessa, DATEPART(yy, dbo.cantiere_registrazioni.data) AS anno, DATEPART(mm, dbo.cantiere_registrazioni.data) 
            AS mese, dbo.cantiere_ponti_ditte_registrazioni.ponte, dbo.cantiere_ponti_ditte_registrazioni.operatore
