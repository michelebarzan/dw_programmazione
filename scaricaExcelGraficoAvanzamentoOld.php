<?php
	...
	echo "<tr>";
			echo "<th>Attivita (codice attivita)</th>";
			echo "<th>Settimana</th>";
			echo "<th>Numero cabine</th>";
	echo "</tr>";
	...
	else
	{
		while($row3=sqlsrv_fetch_array($result3))
		{
			...
			if($tipo=="complessivo")
			{
				$query2="SELECT descrizione, REPLACE(CAST(settimana AS varchar(10)), '.', '_') AS settimana,
							...
							RIGHT({ fn CONCAT('0', CONVERT(varchar(3), DATEPART(ww, dbo.attivitasvoltenoaccento.datasvolgimento))) }, 2)) } < '$settimanaFine') AND (dbo.programmazione_attivita.codice_attivita = ".$row3['codice_attivita'].")
						...
						FROM (SELECT TOP (100) PERCENT programmazione_attivita_1.descrizione, programmazione_attivita_1.codice_attivita, COUNT(*) AS nCabine, { fn CONCAT({ fn CONCAT(CONVERT(varchar(4), 
							...
							CONVERT(varchar(3), DATEPART(ww, attivitasvoltenoaccento_1.datasvolgimento))) }, 2)) } < '$settimanaFine') AND (programmazione_attivita_1.codice_attivita = ".$row3['codice_attivita'].")
							ORDER BY settimana) AS v2
						ORDER BY descrizione, settimana";
			}
			if($tipo=="parziale")
			{
				$query2="SELECT descrizione, REPLACE(CAST(settimana AS varchar(10)), '.', '_') AS settimana,nCabine, codice_attivita
						FROM (SELECT TOP (100) PERCENT programmazione_attivita_1.descrizione, programmazione_attivita_1.codice_attivita, COUNT(*) AS nCabine, { fn CONCAT({ fn CONCAT(CONVERT(varchar(4), 
							YEAR(attivitasvoltenoaccento_1.datasvolgimento)), '.') }, RIGHT({ fn CONCAT('0', CONVERT(varchar(3), DATEPART(ww, attivitasvoltenoaccento_1.datasvolgimento))) }, 2)) } AS settimana
							FROM dbo.programmazione_attivita AS programmazione_attivita_1 INNER JOIN
							dbo.attivitasvoltenoaccento AS attivitasvoltenoaccento_1 INNER JOIN
			...
			else
			{
				...
				{
					echo "<tr>";
						echo "<td>".$row2["descrizione"]." (".$row2["codice_attivita"].")"."</td>";
						echo "<td>".$row2["settimana"]."</td>";
						echo "<td>".$row2["nCabine"]."</td>";
					echo "</tr>";
				}
			}
		}
	...
	else
	{
		while($row4=sqlsrv_fetch_array($result4))
		{
			...
			else
			{
				...
				{
					echo "<tr>";
						echo "<td>".$row5["descrizione"]." (".$row5["codiceAttivita"].")"."</td>";
						echo "<td>".$row5["settimana"]."</td>";
						echo "<td>".$row5["nCabine"]."</td>";
					echo "</tr>";
				}
			}
		}
